...
namespace Vulkan {
     ...
     * Lucky for us, the data is stored in an std::vector where the default allocator already ensures that
     * the data satisfies the worst case alignment requirements.
     */
    ...
    GraphicsPipeline::GraphicsPipeline(const LogicalDevice& logicalDevice, const SwapChain &swapChain)
        ...
        // пошла портянка, НУ ЧО ЕБАНА В РОТ ПОГНАЛИ НАХУЙ
        // https://vulkan-tutorial.com/en/Drawing_a_triangle/Graphics_pipeline_basics/Fixed_functions
        // динамичейский стейт - параметры пайплайны, которые указываются на лету, типа размера экрана, толщины линии (чо?)
